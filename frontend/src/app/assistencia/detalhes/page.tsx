...
export default function DetalhesPage() {
  ...
  const load = () => assistenciaRegistersApi.detalhes.list(search || undefined).then(setItems);
  ...
  const handleCreate = async () => {
    ...
    await assistenciaRegistersApi.detalhes.create(newName.trim());
    setNewName('');
    load();
  };
  ...
  const handleUpdate = async (id: number) => {
    ...
    await assistenciaRegistersApi.detalhes.update(id, editName.trim());
    setEditId(null);
    load();
  };

  const handleDelete = async (id: number) => {
    if (confirm('Excluir detalhe?')) {
      await assistenciaRegistersApi.detalhes.delete(id);
      load();
    }
  };
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-gray-800">Detalhes</h1>

      <div className="flex gap-2">
        <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
          placeholder="Novo detalhe..." className="flex-1 border rounded-lg px-3 py-2 text-sm" />
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            ...
            {items.length === 0 && <tr><td colSpan={2} className="p-4 text-center text-gray-400">Nenhum detalhe cadastrado</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
